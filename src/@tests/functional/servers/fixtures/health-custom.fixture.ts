...
/** Imported before the `bootstrapServers()` call in health-custom-paths-and-checks.test.ts. */
await ProgramModule.applications.define('health-custom', () => {
  @Controller()
  class _HealthCustomController extends ZanixController {
    @Get('/orders')
    public list() {
      return { orders: [] }
    }
