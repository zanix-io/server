 ...
 * @example
 * ```ts
 * class ReplaceUserBody extends BaseRTO {
 *   \@IsEmail()
 *   accessor email!: string
 * }
 *
 * class UsersController extends ZanixController {
 *   \@Put(':id', { Body: ReplaceUserBody })
 *   public replaceUser(ctx: HandlerContext<{ body: ReplaceUserBody }>) {
 ...
 * ```ts
 * class UsersController extends ZanixController {
 *   // Registers on path 'replaceUser', since no path is given
 *   \@Put({ Body: ReplaceUserBody })
 *   public replaceUser(ctx: HandlerContext<{ body: ReplaceUserBody }>) {
 *     return { email: ctx.payload.body.email }
 *   }
 * }
